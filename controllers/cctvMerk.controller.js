...
const getAll = async (req, res) => {
  try {
    ...
    const logs = await CCTVMerk.getAll(limit, offset);
    const total = await CCTVMerk.getCount();
    ...
    console.error("Error getting CCTVMerks:", error);
...
const getById = async (req, res) => {
  try {
    ...
    const merk = await CCTVMerk.getById(id);
    if (!merk) {
...
const update = async (req, res) => {
  try {
    ...
    const existingMerk = await CCTVMerk.getById(id);
    if (!existingMerk) {
      return res.status(404).json({ message: "Merk not found" });
    }

    await CCTVMerk.update(id, {
      name,
    });
...
const deleteCCTVMerk = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const controllers = await CCTVController.getByMerkId(id);
